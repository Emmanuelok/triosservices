...
const allowedPlans = ['annual', 'care-planner', 'winter', 'summer', 'lawn'];
...
export function Booking() {
  ...
  function field<K extends keyof QuoteForm>(key: K, value: QuoteForm[K]) { setForm(previous => ({ ...previous, [key]: value })); }
  ...
  function selectProperty(value: string) { setSelectedProperty(value); const property = data?.properties?.find((item: any) => item.id === value); if (property) setForm(previous => ({ ...previous, address: property.address, area: property.area, postalCode: property.postal_code, details: { ...previous.details, access: property.details || '' } })); }
  function toggleService(service: string) { field('services', form.services.includes(service) ? form.services.filter(value => value !== service) : [...form.services, service]); }
  function packageSelection(services: string[], plan: string) { setForm(previous => ({ ...previous, services, frequency: plan === 'annual' ? 'Mixed / help me choose' : 'Seasonal', details: { ...previous.details, plan } })); }
  ...
  const requestText = ['PROPERTY CARE ENQUIRY — TRIOS SNOW AND MOWING INC.', '', `Name: ${form.name || 'To be supplied'}`, `Email: ${form.email || 'To be supplied'}`, `Phone: ${form.phone || 'To be supplied'}`, `Property: ${form.address || 'To be supplied'}, ${form.area} ${form.postalCode}`, '', `Services: ${selected.map(service => service.name).join(', ') || 'To be selected'}`, `Frequency: ${form.frequency}`, `Preferred first visit: ${form.details.preferredDate || 'To be discussed'}`, ...(snow ? [`Winter season: ${SEASON}`, `Driveway: ${form.details.drivewaySize}; ${form.details.surface}; ${form.details.slope}`, `Salting quote: ${form.details.salt ? 'Yes' : 'No'}`, `Walkways: ${form.details.walkway ? 'Yes' : 'No'}`, `Priority requested: ${form.details.priority ? 'Yes' : 'No'}`, `Usual departure: ${form.details.departureTime || 'Not supplied'}`, `Snow placement: ${form.details.snowStorage || 'To be agreed'}`] : []), ...(form.details.lawnArea ? [`Lawn / garden area: ${form.details.lawnArea}`] : []), `Access notes: ${form.details.access || 'None supplied'}`, '', 'This is a quote enquiry. No booking, price or payment is confirmed.'].join('\n');
  function downloadDraft() { const url = URL.createObjectURL(new Blob([requestText], { type: 'text/plain;charset=utf-8' })); const link = document.createElement('a'); link.href = url; link.download = 'trios-property-care-enquiry.txt'; link.click(); URL.revokeObjectURL(url); }
  ...
  async function submit() {
    ...
    try {
      ...
      const result = await saveData({ ...form, action: 'request', id: id.current, consent });
  ...
  return <><Intro eyebrow="YOUR PROPERTY. YOUR KIND OF CARE." title="Let’s make your day a little easier.">Choose what you need, share the details, and review your tailored quote before work is confirmed.</Intro><div className="container page-body customer-upgrade">
    ...
    <div className="stepper" aria-label="Quote request progress">{['Choose your care', 'Your property', 'Review & request'].map((label, index) => <div key={label} aria-current={step === index ? 'step' : undefined} className={'step' + (step >= index ? ' active' : '')}>{String(index + 1).padStart(2, '0')} / {label}</div>)}</div>
