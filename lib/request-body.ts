...
export async function boundedBody(req:Request,max:number):Promise<Uint8Array>{
 ...
 try{while(true){const {done,value}=await reader.read();if(done)break;length+=value.byteLength;if(length>max){await reader.cancel();throw new BodyError('Request is too large.',413)}chunks.push(value)}}finally{reader.releaseLock()}
...
export async function boundedJson(req:Request,max=50000){try{return JSON.parse(new TextDecoder().decode(await boundedBody(req,max)))}catch(error){if(error instanceof BodyError)throw error;throw new BodyError('Please submit a valid form.',400)}}
