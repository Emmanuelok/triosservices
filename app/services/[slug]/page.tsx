...
export default async function Service({params}:{params:Promise<{slug:string}>}){const {slug}=await params;if(slug==='moving')permanentRedirect('/moving');if(!SERVICES.some(s=>s.id===slug))notFound();return <SiteApp page="service" serviceId={slug}/>}
