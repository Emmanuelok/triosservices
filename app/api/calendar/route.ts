...
export async function GET(req:Request){try{
 ...
 const {results}=await sql().prepare(`SELECT j.*,r.address,r.area,r.details FROM jobs j JOIN requests r ON r.id=j.request_id WHERE ${where} AND j.status!='cancelled' ORDER BY j.scheduled_date LIMIT 1000`).bind(...bindings).all();
 ...
 for(const j of results){const move=j.service==='moving'?cleanMoving(JSON.parse(j.details||'{}').moving):null;const movingNote=move?'\nMoving tier: '+(MOVING_TIERS.find(t=>t.id===move.tier)?.name||'To confirm')+'\nDestination: '+move.destination.address+'\nReview the private move workspace for inventory, access and latest transport arrangements.':'';const end=new Date(j.scheduled_date+'T12:00:00Z');end.setUTCDate(end.getUTCDate()+1);lines.push('BEGIN:VEVENT','UID:'+j.id+'@trios','DTSTAMP:'+stamp,'DTSTART;VALUE=DATE:'+j.scheduled_date.replaceAll('-',''),'DTEND;VALUE=DATE:'+end.toISOString().slice(0,10).replaceAll('-',''),'SUMMARY:'+escape(SERVICES.find(s=>s.id===j.service)?.name||j.service),'LOCATION:'+escape(j.address+', '+j.area),'DESCRIPTION:'+escape('Agreed window: '+j.time_window+' (St. John’s time).\nStatus: '+j.status+'\n'+j.notes+movingNote),'TRANSP:TRANSPARENT','END:VEVENT')}
